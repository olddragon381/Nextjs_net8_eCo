...
const Dropdown = ({ menuItem, stickyMenu }) => {
  ...
  return (
    ...
    >
      <a
        href="#"
        className={`hover:text-blue text-custom-sm font-medium text-dark flex items-center capitalize ${
          stickyMenu ? "" : "xl:py-1"
        } ${pathUrl.includes(menuItem.title) && "!text-blue"}`}
      >
       <span
    title={menuItem.title} // tooltip khi hover
    className="inline-block font-medium  max-w-[80px] truncate overflow-hidden whitespace-nowrap"
  >
    {menuItem.title}
  </span>
        <svg
