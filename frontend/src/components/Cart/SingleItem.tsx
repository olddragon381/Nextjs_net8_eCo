...
import { removeIteminCart } from "@/service/cart/CartService";
...
const SingleItem = ({ item }) => {
  ...
  const handleRemoveFromCart = () => {
       ...
       try {
        removeIteminCart(token,item.id);
        dispatch(removeItemFromCart(item.id));
        console.log("✅ Xoá thành công khỏi server và local");
        }catch (err) {
  ...
  return (
    <div className="flex items-center border-t border-gray-3 py-5 px-7.5">
      ...
      <div className="min-w-[200px]">
        <p className="text-dark">${item.price * quantity + ".000đ"}</p>
      </div>
