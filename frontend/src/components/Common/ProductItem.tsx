  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        <Image
  src={item.image?.startsWith("http") || item.image?.startsWith("/") ? item.image : "/images/default.jpg"}
  alt={item.title}
  width={250}
  height={250}
...
/>
      ...
      <div className="flex items-center gap-2.5 mb-2">
         <div className="flex items-center gap-1">
                    ...
                    {[...Array(5)].map((_, index) => {
                      const isFullStar = index + 1 <= Math.floor(item.rating);
                      const isHalfStar =
                        index + 1 > Math.floor(item.rating) &&
                        index + 1 - item.rating < 1;
      ...
      >
        ...
        <Link href={`/shop-details/${item.id}`} key={item.id}> {item.title} </Link>
      </h3>

      <span className="flex items-center gap-2 font-medium text-lg">
     
        <span className="text-dark-4 ">{item.price + ".000đ"}</span>
      </span>
    </div>
  );
};
