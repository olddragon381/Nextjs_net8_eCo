...
const CustomToast = ({ t, message, type }) => {
  ...
  return (
    <div
      className={`fixed flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow-sm top-45 dark:text-gray-400 dark:bg-gray-800 ${
        t.visible ? 'animate-enter' : 'animate-leave'
      }`}
      role="alert"
    >
      <div className={`inline-flex items-center justify-center shrink-0 w-8 h-8 rounded-lg ${icon.color}`}>
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">{icon.svg}</svg>
