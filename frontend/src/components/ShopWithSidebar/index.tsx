...
import { fetchAllCategories } from "@/service/category/CategoryService";
import { Category } from "@/types/category";
import { fetchBooks } from "@/service/products/ProductsServices";
...
const ShopWithSidebar = () => {
  ...
  const [totalPages, setTotalPages] = useState(1); // Nếu API trả về total
...
for (let i = startPage; i <= endPage; i++) {
  ...
  const options = [
    { label: "Chọn theo yêu cầu ", value: "0" },
    { label: "Mới cập nhập", value: "1" },
    { label: "Được đánh giá cao", value: "2" },
    { label: "Được chú ý nhiều", value: "3" },
    { label: "Giá tăng dần", value: "4" },
 ...
 const loadBooks = async () => {
  try {
    const result = await fetchBooks(page, pageSize, genres ?? []);
  ...
  useEffect(() => {
    if (genre) {
      // Nếu người dùng đi từ header, set genres tương ứng
      setGenres([genre as String]);
      setPage(1); // reset lại trang
    }
  }, [genre]);

  useEffect(() => {
     console.log("genres changed:", genres);
  loadBooks();
}, [page, genres])
    useEffect(() => {
  ...
  return () => {
    ...
    useEffect(() => {
  fetchAllCategories().then((categories: Category[]) => {
...
const sortedBooks = useMemo(() => {
    ...
    return (
      book.price >= selectedPrice.from &&
      book.price <= selectedPrice.to
    );
  });
    switch (sortType) {
                  ...
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <div className="flex items-center justify-between">
                      <p>Filters:</p>
                              ...
                              onClick={() => {
                                setGenres([]);
                                setSelectedPrice({ from: 0, to: maxBookPrice });
                                setPage(1);
                              }}
                              className="text-blue"
                            >
  ...
  value={genres} // <-- truyền state cha xuống
...
/>
                  ...
                  {/* // <!-- price range box --> */}
                  <PriceDropdown maxPrice={maxBookPrice}
...
/>
              ...
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <CustomSelect options={options} onChange={(value) => setSortType(value)}/>

                   Hiển thị {(page - 1) * pageSize + 1}–{Math.min(page * pageSize, totalPages)} trong {totalPages} trang
                  </div>
  ...
  <li>
    <button
      ...
      className="w-8 h-9 rounded hover:bg-blue hover:text-white disabled:text-gray-400"
    >
      &lt;
    </button>
  </li>
  ...
  {pageNumbers.map((p, idx) =>
    p === "prevDots" || p === "nextDots" ? (
      <li key={idx} className="w-8 h-9 flex items-center justify-center">…</li>
    ) : (
      <li key={p}>
        <button
          onClick={() => setPage(p)}
          className={`w-8 h-9 rounded ${
            p === page ? "bg-blue text-white" : "hover:bg-gray-200"
          }`}
        >
          {p}
        </button>
